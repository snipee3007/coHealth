...
exports.createAppointment = catchAsync(async (req, res, next) => {
    ...
    const matchDoctor = doctors.find(doctor => doctor.doctorInfo[0] && doctor.doctorInfo[0].major === req.body.specialty)
...
exports.getAllAppointment = catchAsync(async (req, res, next) => {
    const appointments = await Appointment.find({
        time: {$gte: Date.now()}
    }).populate({path: 'doctorID', select: "fullname", populate: ({path: 'doctorInfo'})})
    res.status(200).json({
        message: "success",
