...
function fadeOutContent() {
    document.querySelector("#appointment-container").classList.remove("opacity-100")
    document.querySelector("#appointment-container").classList.add("opacity-0")
}

function fadeInContent() {
    document.querySelector("#appointment-container").classList.remove("opacity-0")
...
class ListAppointment {
    ...
    #loadAppointments(){
        async function showPage(page){
            const res = await fetch(`/api/appointment/list/get?page=${page}`,{
                method: 'GET',
                headers: {
            ...
            appointments.forEach((appointment) => {
                ...
                let name = document.querySelector('.role').textContent === 'Doctor'? `${appointment.doctorID.fullname}` : `${appointment.fullname}`
                ...
                let details = appointment.status == -1? `<span class="inline-block bg-gray-200 rounded-xl px-4 py-2">Unchecked</span>`
                : appointment.status == 1? `<span class='inline-block text-[#4461F2] bg-white rounded-xl px-4 py-2'>Accepted</span>`
                : `<span class="bg-white rounded-xl p-4 text-red-300">Decline</span>`
                document.querySelector('#appointment-container').insertAdjacentHTML('beforeend',`
                    <div class="flex mt-4 p-4 justify-between space-x-4 text-center">
                        <p class="text-xl my-4 w-1/5">${name}</p>
                        <p class="text-xl my-4 w-1/5"> ${days[createdDate.getDay()]}, ${months[createdDate.getMonth()]} ${date} ${createdDate.getFullYear()} </p>
                        <p class="text-xl my-4 w-1/5">
                            ${ createdDate.getHours() === 9? '09': createdDate.getHours()} : ${createdDate.getMinutes() === 0? '00': '30' }
                        </p>
                        <p class="text-xl my-4 w-1/5">
                            ${details}
                        </p>
                        <a href='./list/${appointment.appointmentCode}' class='text-xl my-4 w-1/5 text-[#4461F2] underline underline-offset-2'>
                            Details
                        </p>
    ...
    #changeColorSetting(){
        ...
        document.querySelector('a.appointment img').src = "./../images/png/calendar-active.png"
        document.querySelector('a.profile img').src = "./../images/png/icon-inactive.png"
    }
